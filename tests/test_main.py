...
fullPathOfImages = [
    os.path.join(testImagesDir, name) for name in namesOfTestEverythingImages
]
fullPathOfWritingTestImages = [
    os.path.join(testImagesDir, "TestImagesWriting", "WritingTest_" + name)
    for name in namesOfTestEverythingImages
]

parameterizedSetup = [
    {"imagePath": path} for path in fullPathOfImages + fullPathOfWritingTestImages
]
parameterizedSetupWithoutWritingTest = [
    {"imagePath": path} for path in fullPathOfImages
]


@parameterized_class(parameterizedSetup)
class Test02OnImagesRead(unittest.TestCase):
    ...
    def test_ImagePixelValues(self):
        ...
        self.assertEqual(
            self.ImageClass.PixelValues.shape,
            (self.ImageClass.Height, self.ImageClass.Width, self.ImageClass.Bands),
        )
        ...
        if (
            self.ImageClass.ImageFileName
            == "WritingTest_TestEverythingImage_VeryHighCompression.hips"
        ):
            return
        self.assertTrue(
            np.all(
                np.round(self.ImageClass.PixelValues[:, :, 0])
                == np.array([[0, 1, 2], [3, 4, 5]], dtype=np.float32)
            )
        )
        self.assertEqual(
            np.sum(np.round(self.ImageClass.PixelValues), dtype=int),
            np.sum([1, 2, 3, 4, 5]),
        )
    ...
    def test_Mask(self):
        ...
        self.assertTrue(
            np.all(
                self.ImageClass.ForegroundPixels
    ...
    def test_CorrectedPixels(self):
        ...
        self.assertTrue(
            np.all(
                self.ImageClass.CorrectedPixels
                == np.array([[0, 1, 1], [1, 1, 1]], dtype=np.int32)
            )
        )
    ...
    def test_FreehandLayers(self):
        ...
        freehandLayers = sorted(
            self.ImageClass.FreehandLayers, key=lambda d: d["layerId"]
        )
        ...
        self.assertTrue(
            np.all(
                freehandLayers[1]["pixels"]
                == np.array([[1, 0, 0], [0, 0, 0]], dtype=np.int32)
            )
        )
    ...
    def test_SaturatedPixels(self):
        ...
        self.assertTrue(
            np.all(
                self.ImageClass.SaturatedPixels
                == np.array([[0, 0, 0], [0, 0, 1]], dtype=np.int32)
            )
        )
    ...
    def test_MetaData_StrobeTimes(self):
        ...
        self.assertTrue(
            np.all(self.ImageClass.StrobeTimes == np.arange(19, 0, -1, dtype=np.int32))
        )
    ...
    def test_MetaData_BandNames(self):
        ...
        self.assertTrue(
            np.all(
                self.ImageClass.BandNames
                == np.array(["BandName" + str(i) for i in range(1, 20)])
            )
        )
    ...
    def test_MetaData_WaveLengths(self):
        ...
        self.assertTrue(
            np.all(
                self.ImageClass.WaveLengths == np.array([100 + i for i in range(1, 20)])
            )
        )
    ...
    def test_MetaData_UniversalStrobeTimes(self):
        ...
        self.assertTrue(
            np.all(
                self.ImageClass.StrobeTimesUniversal == (20.0 + np.arange(18, -1, -1))
            )
        )
            ...
            np.all(
                ...
                == np.array(
                    [
                        ...
                        "Direct_Lowpower_LED",
                        "Diffused_UV",
                        "Harmless_Guide_Laser",
                        "TEST_PROBE",
                        "Darkfield_FrontLight",
                        "Coaxial_FrontLight",
                        "NA",
                        "NA",
                        "NA",
    ...
    def test_MetaData_MmPixel_Description_History_CompressionMode(self):
        ...
        self.assertEqual(
            self.ImageClass.History.replace("\r\n", ""), "History from the test image"
        )

        compressionOfTestImage = self.ImageClass.ImageFileName.split("_")[-1].replace(
            ".hips", ""
        )
...
class Test01OnImagesWrite(unittest.TestCase):
    ...
    def test_WriteImageClass(self):
        path = hips.write(
            self.ImageClass,
            os.path.join(
                "TestImagesWriting", "WritingTest_" + self.ImageClass.ImageFileName
            ),
        )
    ...
    def test_ImageShapeExceptionsWhenSaving(self):
        ...
        self.assertRaises(
            ValueError, hips.write, self.ImageClass, "test_PixelValues.hips"
        )  # Error, func, arg1, arg2
        ...
        self.assertRaises(
            ValueError, hips.write, self.ImageClass, "test_CorrectedPixels.hips"
        )  # Error, func, arg1, arg2
        ...
        self.assertRaises(
            ValueError, hips.write, self.ImageClass, "test_DeadPixels.hips"
        )  # Error, func, arg1, arg2
        ...
        self.assertRaises(
            ValueError, hips.write, self.ImageClass, "test_ForegroundPixels.hips"
        )  # Error, func, arg1, arg2
        ...
        self.assertRaises(
            ValueError, hips.write, self.ImageClass, "test_SaturatedPixels.hips"
        )  # Error, func, arg1, arg2
...
class Test03OnImagesVMfunctions(unittest.TestCase):
    ...
    def test_show(self):
        ...
        self.assertTrue(
            np.all(
                titles
                == np.array(
                    ["Band " + str(i + 1) for i in range(self.ImageClass.Bands)]
                )
            )
        )
    ...
    def test_showReduced(self):
        ...
        plt_outputs = hips.show(
            self.ImageClass,
            bandIndexesToUse=bandIndexesToUse,
        ...
        self.assertEqual(
            self.ImageClass.Bands, 19
        )  ##  check if this changed something within the ImageClass
    ...
    def test_showMasked(self):
        ...
        plt_outputs = hips.show(
            self.ImageClassMasked, ifUseMask=True, ifOnlyGetListOfPLTObjects=True
        )
        plt.close("all")
        images = np.array([p.get_array() for p in plt_outputs])
        titles = np.array([p.axes.get_title() for p in plt_outputs])
        ...
        self.assertTrue(
            np.all(
                titles
                == np.array(
                    ["Band " + str(i + 1) for i in range(self.ImageClassMasked.Bands)]
                )
            )
        )
    ...
    def test_ReduceBands(self):
        ...
        self.assertTrue(
            np.all(
                self.ImageClassReduced.PixelValues.shape
                == (self.ImageClassReduced.Height, self.ImageClassReduced.Width, bands)
            )
        )
        self.assertEqual(self.ImageClassReduced.Bands, bands)
        self.assertTrue(
            np.all(self.ImageClassReduced.BandNames == ["BandName1", "BandName19"])
        )
        self.assertTrue(
            np.all(
                self.ImageClassReduced.WaveLengths
                == np.array([101.0, 119.0], dtype=np.float32)
            )
        )
        self.assertTrue(np.all(self.ImageClassReduced.StrobeTimes == np.array([19, 1])))
        self.assertTrue(
            np.all(
                self.ImageClassReduced.StrobeTimesUniversal
                == np.array([38.0, 20.0], dtype=np.float32)
            )
        )
        self.assertTrue(np.all(self.ImageClassReduced.Illumination == ["Mixed", "NA"]))

        if (
            "Uncompressed" in self.ImageClassReduced.ImageFileName
        ):  ## Uncompressed is not initialized correctly in the Compression Preset LUT
        ...
        self.assertTrue(
            np.all(
                np.round(self.ImageClassReduced.PixelValues[:, :, 0])
                == np.array([[0, 1, 2], [3, 4, 5]], dtype=np.float32)
            )
        )
        self.assertEqual(
            np.sum(np.round(self.ImageClassReduced.PixelValues), dtype=int),
            np.sum([1, 2, 3, 4, 5]),
        )
...
class TestWriting(unittest.TestCase):
    ...
    def test_WriteNpArray(self):
        ...
        path = hips.write(
            arr, os.path.join("TestImagesWriting", "WritingTest_Arr.hips")
        )
        self.assertIsNotNone(path)
        self.assertTrue(os.path.isfile(path))
...
class TestHelperFunctions(unittest.TestCase):
    ...
    def test_npArray2VMImage(self):
        ...
        # 2-D
        vmImg2 = utils.npArray2VMImage(
            np.array(
                [[5.81, 6.10, 5.29], [6.15, 6.13, 5.22], [6.33, 5.90, 5.01]],
                dtype=np.float32,
            )
        )

        # 3-D
        vmImg3 = utils.npArray2VMImage(
            np.array(
                [[5.81, 6.10, 5.29], [6.15, 6.13, 5.22], [6.33, 5.90, 5.01]],
                dtype=np.float32,
            ).reshape((3, 3, 1))
        ...
        for i in range(3):
            for j in range(3):
                self.assertTrue(
                    abs(vmImgRef.GetPixel(i, j, 0) - vmImg2.GetPixel(i, j, 0))
                    < diffLimit
                )
                self.assertTrue(
                    abs(vmImgRef.GetPixel(i, j, 0) - vmImg3.GetPixel(i, j, 0))
                    < diffLimit
                )
